...
_Persisch für „der Ausschau hält / Beobachtungsposten"._
...
Jede überwachte Website ist ein **Projekt** — eine Datei in
`worker/projects/`. Der Dateiname ist die Kennung und zugleich der Ordner
unter `data/`:

```json
{
    "name": "Example",
    "source": { "type": "manual", "urlsFile": "./projects/example.urls.txt" },
    "notify": ["ich@example.com"],
    "crawlDelayMs": 500
}
```

`source` ist entweder `{ "type": "sitemap", "sitemapUrl": "…" }` oder
`{ "type": "manual", "urlsFile": "…" }` bzw. `{ "type": "manual", "urls": [...] }`.
Jedes weitere Feld überschreibt `didban.config.json` nur für dieses Projekt.
`notify` wird gespeichert und im Report ausgewiesen — **verschickt wird noch
nichts**, das kommt mit Phase 4.

Details: [`worker/projects/README.md`](worker/projects/README.md).
...
# ohne Projekt, zum Ausprobieren
docker compose run --rm worker npm run check -- --urls https://example.com/
```

Ergebnis je Lauf unter `data/<projekt>/`:
